...
public class ChatPresenter implements ChatMVP.Presenter, TIMMessageListener {
    ...
    public void recordSound() {
        ...
        mSoundPath = BaseApplication.getContext().getFilesDir() + File.separator + mIdentifier +
    ...
    public void sendSoundMessage(boolean needToSend) {
        ...
        if (needToSend) {
            if (duration < 1) {
                Toast.makeText(BaseApplication.getContext(), "录音时间过短", Toast.LENGTH_SHORT).show();
                return;
            }
            TIMMessage timMessage = new TIMMessage();
            ...
            if(timMessage.addElement(soundElem) != 0) {
                Toast.makeText(BaseApplication.getContext(), "发送失败", Toast.LENGTH_SHORT).show();
                return;
            }
            view.getAdapterData().add(0,timMessage);
            view.updateRecyclerView();
