...
import com.example.libmedia.MediaUtils;
import com.example.libmedia.R;
import com.example.libmedia.utils.FileUtil;

/**
 * A {@link org.wordpress.mediapicker.source.MediaSource} that loads images from
 * the device {@link android.provider.MediaStore}.
 */

public class MediaSourceDeviceImages implements MediaSource {
	// Columns to query from the thumbnail MediaStore database
	private static final String[] THUMBNAIL_QUERY_COLUMNS = { MediaStore.Images.Thumbnails._ID,
	...
	private static final String[] IMAGE_QUERY_COLUMNS = { MediaStore.Images.Media._ID, MediaStore.Images.Media.DATA,
			MediaStore.Images.Media.DATE_TAKEN, MediaStore.Images.Media.ORIENTATION };

	protected final List<MediaItem> mMediaItems;
	protected final List<MediaItem> mMediaItemsSelected;
	...
	public MediaSourceDeviceImages() {
		...
		mMediaItemsSelected = new ArrayList<>();
	}
	...
	protected List<MediaItem> createMediaItems() {
		...
		return MediaUtils.createMediaItems(thumbnailData, MediaStore.Images.Media.query(mContext.getContentResolver(),
				MediaStore.Images.Media.EXTERNAL_CONTENT_URI, IMAGE_QUERY_COLUMNS, null, null,
				MediaStore.MediaColumns.DATE_MODIFIED + " DESC"), MediaUtils.BackgroundFetchThumbnail.TYPE_IMAGE);
	}
	...
	public void cleanup() {
		if (mGatheringTask != null) {
			// cancel gathering media data immediately, do not wait for the task
			// to finish
			mGatheringTask.cancel(true);
		}
		mMediaItems.clear();
	...
	public View getView(int position, View convertView, ViewGroup parent, LayoutInflater inflater,
			final ImageLoader.ImageCache cache) {
		if (convertView == null) {
		...
		if (convertView != null && mediaItem != null) {
			View rlMark = convertView.findViewById(R.id.rl_mark);
			if (mMediaItemsSelected.contains(mediaItem)) {
//				rlMark.setTag("mark" + position);
...
//			}
			if(mediaItem.getPreviewSource() != null&&FileUtil.checkFilePathExists(mediaItem.getPreviewSource().toString())){
				imageSource = mediaItem.getPreviewSource();
			}else{
				imageSource = mediaItem.getSource();
			}
				...
				if (width <= 0 || height <= 0) {
					imageView.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
						...
						public boolean onPreDraw() {
							int width = imageView.getWidth();
							int height = imageView.getHeight();
							MediaUtils.fadeMediaItemImageIntoView(imageSource, cache, imageView, mediaItem, width,
									height, MediaUtils.BackgroundFetchThumbnail.TYPE_IMAGE);
	...
	public boolean onMediaItemSelected(MediaItem mediaItem, boolean selected) {
		if (selected) {
			mMediaItemsSelected.add(mediaItem);
		} else {
			mMediaItemsSelected.remove(mediaItem);
		}
		return !selected;
	}
	...
	protected class GatherDeviceImagesTask extends AsyncTask<Void, String, Void> {
		...
		/**
		 * Sets MediaSourceDeviceImages.this.mGatheringTask to null if it's
		 * referencing this.
		 */
