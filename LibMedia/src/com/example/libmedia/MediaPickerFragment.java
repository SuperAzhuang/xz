...
/**
 * MediaPickerFragment tracks a collection of MediaSources and responds to
 * changes via the
 * {@link org.wordpress.mediapicker.source.MediaSource.OnMediaChange} interface.
 * 
 * If the host Activity implements
 * {@link org.wordpress.mediapicker.MediaPickerFragment.OnMediaSelected} it will
 * automatically be set as the listener, otherwise a setter is provided. This
 * interface is how user intent is delivered, while not having a listener won't
 * break anything it's not very useful.
 * 
 * By default the {@link org.wordpress.mediapicker.MediaSourceAdapter} is shown
 * within a {@link android.widget.GridView}, but a subclass of
 * {@link android.widget.AbsListView} may be provided with
 * id=media_adapter_view. A subclass of {@link android.widget.TextView} may also
 * be provided for the empty view; id=media_empty_view. You can even provide a
 * subclass of {@link org.wordpress.mediapicker.MediaSourceAdapter}.
 * 
 * Menu items may be provided for Action Mode and their selection will be
 * alerted with onMenuItemSelected. A selection confirmation button is
 * automatically added and will call onMediaSelectionConfirmed when selected.
 */

public class MediaPickerFragment extends Fragment implements AdapterView.OnItemClickListener,
		AbsListView.MultiChoiceModeListener, MediaSource.OnMediaChange {
	...
	/**
	 * Interface to respond to user intent and provide a caching mechanism for
	 * the fragment.
	 */
	...
	// Required state tracking to prevent OnMediaSelectionCancelled from being
	// called erroneously
	...
	// Customizable view resources, some default behavior is defined as
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// If this is not being restored from a previous state use arguments to
		// set state
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		View rlMark = view.findViewById(R.id.rl_mark);
		if (mSelectedContent.contains(item)) {
			mSelectedContent.remove(item);
			notifyMediaSelected(position, false);
			rlMark.setVisibility(View.GONE);

		} else {
			if (mSelectedContent.size() >= maxCount) {
				Toast.makeText(getActivity(), "图片不能超过" + maxCount + "张:", Toast.LENGTH_SHORT).show();
			} else {
	...
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
		inflater.inflate(R.menu.media_picker_action_mode, menu);
		MenuItem item = menu.findItem(R.id.menu_media_selection_confirmed);
		item.setShowAsAction(ActionMenuItem.SHOW_AS_ACTION_ALWAYS);
		super.onCreateOptionsMenu(menu, inflater);
	}

	@Override
	public boolean onOptionsItemSelected(MenuItem item) {
		if(item.getItemId()==R.id.menu_media_selection_confirmed){
	...
	/**
	 * Sets the listener. Calling this method will overwrite the current
	 * listener.
	 * 
	 * @param listener
	...
	/**
	 * Sets the {@link org.wordpress.mediapicker.source.MediaSource}'s to be
	 * presented. The current sources and selected content are cleaned up and
	 * cleared before the new list added.
	 * 
	 * @param mediaSources
	 *            the new sources
	 */
	 ...
	 * 
	 * @param id
	 *            the ID of the menu resource, any value < 0 will use the
	 *            default menu
	 */
	 ...
	 * 
	 * @param customLayout
	 *            the ID of the layout resource, any value < 0 will use the
	 *            default layout
	 */
	...
	/**
	 * Same as calling {@link #setLoadingText(String)} with
	 * {@link #getString(int)} for resId. Passing resId < 0 will set the text to
	 * null.
	 * 
	 * @param resId
	...
	/**
	 * Same as calling {@link #setEmptyText(String)} with
	 * {@link #getString(int)} for resId. Passing resId < 0 will set the text to
	 * null.
	 * 
	 * @param resId
	 ...
	 * 
	 * @param errorText
	 *            the text to display when an error occurs while loading, can be
	 *            null
	 */
	...
	/**
	 * Same as calling {@link #setErrorText(String)} with
	 * {@link #getString(int)} for resId. Passing resId < 0 will set the text to
	 * null.
	 * 
	 * @param resId
	 *            resource ID of the string to display when there is an error
	 *            loading media
	 */
	...
	/**
	 * Restores state from a given {@link android.os.Bundle}. Checks for media
	 * sources, selected content, custom view, custom action mode menu, and
	 * custom empty text.
	 * 
	 * @param bundle
	...
	private void restoreFromBundle(Bundle bundle) {
		if (bundle != null) {
			...
			if(bundle.containsKey(KEY_MAX_COUNT)){
	...
	/**
	 * Sets the default empty text strings if they are not already set to
	 * something.
	 */
	...
	/**
	 * Constructs the {@link org.wordpress.mediapicker.MediaSourceAdapter} and
	 * attaches it to the adapter view if possible.
	 */
	...
	/**
	 * Calls
	 * {@link org.wordpress.mediapicker.source.MediaSource.OnMediaChange#cleanup()}
	 * on all non-null sources.
	 */
	...
	/**
	 * Creates the {@link org.wordpress.mediapicker.MediaSourceAdapter} and
	 * initializes the adapter view to display it.
	 */
	private void layoutAdapterView() {
		...
		mAdapterView.setChoiceMode(AbsListView.CHOICE_MODE_NONE);
		mAdapterView.setScrollBarStyle(View.SCROLLBARS_OUTSIDE_OVERLAY);
	}

	/**
	 * Adds a menu item to confirm media selection during Action Mode. Only adds
	 * one if one is not defined.
	 * 
	 * @param menu
	...
	private void addSelectionConfirmationButtonMenuItem(Menu menu) {
		if (menu != null && menu.findItem(R.id.menu_media_selection_confirmed) == null) {
			menu.add(Menu.NONE, R.id.menu_media_selection_confirmed, Menu.FIRST, R.string.confirm).setIcon(
					R.drawable.action_mode_confirm_checkmark);
		}
	}

	/**
	 * If the current adapter does not have any items the empty view will be
	 * shown and the adapter view will be hidden. Otherwise the empty view will
	 * be hidden and the adapter view presented.
	 */
